        ...
        public void BuscarPorNombre(String busqueda)
        {
            var consulta = from c in contexto.Clientes
                           ...
                           select new
                           {
                               ...
                               Correo = (from e in c.Correos where e.Principal select e.Direccion).FirstOrDefault() ?? "",
                               Telefono = (from t in c.Telefonos where t.Principal select t.NumeroTelefonico).FirstOrDefault() ?? "",
                               Direccion = (from d in c.Direcciones where d.Principal select d.Calle + " " + d.NumExterior + " " + d.Colonia).FirstOrDefault() ?? ""
                           };
            Clientes.Clear();
            if (consulta != null)
            {
